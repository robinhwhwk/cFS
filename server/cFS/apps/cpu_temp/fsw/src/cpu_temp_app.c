...
void CPU_TEMP_AppMain(void)
{
    ...
    */
    if ((status = CFE_SB_Subscribe(CFE_SB_ValueToMsgId(CPU_TEMP_SENDER_MID), CPU_TEMP_AppData.Pipe)) != CFE_SUCCESS)
    {
        CFE_EVS_SendEvent(CPU_TEMP_EID, CFE_EVS_EventType_ERROR, "error subscribing to command");
        return;
    }
...
void FILE_TRANSFER_CLIENT_ProcessGroundCommand(CFE_SB_Buffer_t *SBBufPtr, int iterations) {
    ...
    for (int i = 0; i < iterations; ++i) {
        ...
        if (FILE_TRANSFER_CLIENT_RequestFile("/dev/virtual_temp", last) != CFE_SUCCESS) {
            CFE_EVS_SendEvent(CPU_TEMP_EID, CFE_EVS_EventType_ERROR,
