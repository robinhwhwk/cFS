...
void FIB_AppMain(void)
{
    ...
    CFE_EVS_SendEvent(FIB_EID, CFE_EVS_EventType_INFORMATION, "Fib subscribed to command pipe FIB_CMD_MID 0x%04X", FIB_CMD_MID);
    ...
    while (CFE_ES_RunLoop(&Fib_AppData.RunStatus) == true)
    {
        ...
        if (status == CFE_SUCCESS)
        {
            ...
            CFE_EVS_SendEvent(FIB_EID, CFE_EVS_EventType_INFORMATION, "Fib received message");
